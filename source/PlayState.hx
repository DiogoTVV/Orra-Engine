...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		scoreTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 150, healthBarBG.y + 26, 0, "", 20);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 22, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
	...
	private function generateStaticArrows():Void
	{
		...
		*/
		...
		// xPos, character, downscroll, isPlayer, botplay, ?size (default is 0.7)
		boyfriendStrums = new Strumline(placement + (mid ? 0 : (FlxG.width / 4)), [boyfriend], FlxG.save.data.downscroll, true, false);
		boyfriendStrums.cameras = [camHUD];
		
		dadStrums = new Strumline(placement - (FlxG.width / (mid ? 3 : 4)), [dad, gf], FlxG.save.data.downscroll, false, true, (mid ? 0.45 : 0.7));
		dadStrums.cameras = [camHUD];
		if(mid)
			for(strum in dadStrums.receptors.members)
				strum.y += ((160 * 0.45) * 1.5) * (dadStrums.downscroll ? -1 : 1);
	...
	override public function update(elapsed:Float)
	{
		...
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, 0.1))); // 0.5
		...
		{
			if (unspawnNotes[0].strumTime - Conductor.songPosition < 4500) // 1500
			{
				var dunceNote:Note = unspawnNotes[0];
				//notes.add(dunceNote);
				var daStrum = strumLines.members[!dunceNote.mustPress ? 0 : 1];
				
				daStrum.push(dunceNote);
				dunceNote.strumline = daStrum;
	...
	function noteCalls(elapsed:Float):Void
	{
		// modchart
		arrowLerp += elapsed * Math.PI * 1.75;
		for(strumline in strumLines)
		{
			if(SONG.song.toLowerCase() == 'fresh')
			{
				...
				{
					strum.x = strum.initialX - Math.sin(arrowLerp + strum.ID) * 30;
					strum.y = strum.initialY - Math.cos(arrowLerp + strum.ID) * 30;
					
					var mult:Float = 0.7 - Math.sin(arrowLerp + strum.ID) * 0.2;
		...
		if (generatedMusic)
		{
			for(strumline in strumLines)
			{
				strumline.allNotes.forEachAlive(function(daNote:Note)
				{
					...
					if(strumline.autoplay && daNote.wasGoodHit && !daNote.mustMiss)
					{
						if (SONG.song != 'Tutorial')
							camZooming = true;
						
						daNote.wasGoodHit = false;
						goodNoteHit(strumline, daNote);
						//trace('it should work');
						return;
					}
					
					var downscrollMult:Float = 0.45;
					if(strumline.downscroll)
						downscrollMult = -0.45;
						
					//daNote.noteVisualOffset = Math.sin(Math.abs(daNote.strumTime - Conductor.songPosition) / 80) * 30;
						
					var daReceptor = strumline.receptors.members[daNote.noteData];
					
					daNote.x = daReceptor.x + daNote.noteVisualOffset;
					daNote.y = (daReceptor.y - (Conductor.songPosition - daNote.strumTime) * (downscrollMult * FlxMath.roundDecimal(SONG.speed, 2)));
					...
					if (daNote.y < -daNote.height && !strumline.downscroll || daNote.y >= daReceptor.y + 106 && strumline.downscroll)
					{
	...
	private function popUpScore(strumline:Strumline, note:Note):Void
		{
			...
			if (daRating == 'sick')
			{
				...
				strumline.noteSplashes.members[note.noteData].animation.play('anim' + FlxG.random.int(1,2), true);
			}
		...
		for(strumline in strumLines)
		{
			...
			if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
			{
				...
				strumline.allNotes.forEachAlive(function(daNote:Note)
				{
					...
					if (daNote.canBeHit && strumline.isPlayer && !daNote.tooLate)
					{
				...
				if (possibleNotes.length > 0)
				{
					...
					if (perfectMode)
						noteCheck(true, strumline, daNote);
						...
						if (possibleNotes[0].strumTime == possibleNotes[1].strumTime)
						{
							for (coolNote in possibleNotes)
							{
								if (controlArray[coolNote.noteData])
									goodNoteHit(strumline, coolNote);
								else
								{
						...
						else if (possibleNotes[0].noteData == possibleNotes[1].noteData)
						{
							if (loadRep)
							{
								if (NearlyEquals(daNote.strumTime,rep.replay.keyPresses[repPresses].time, 30))
								{
									...
									noteCheck(controlArray[daNote.noteData], strumline, daNote);
							}
							else
								noteCheck(controlArray[daNote.noteData], strumline, daNote);
						}
						else
						{
							for (coolNote in possibleNotes)
							{
								if (loadRep)
									...
									noteCheck(controlArray[coolNote.noteData], strumline, daNote);
							}
						}
					}
					else // regular notes?
			...
			if ((up || right || down || left) && generatedMusic || (upHold || downHold || leftHold || rightHold) && loadRep && generatedMusic)
			{
				strumline.allNotes.forEachAlive(function(daNote:Note)
				{
					...
					if (daNote.canBeHit && strumline.isPlayer && daNote.isSustainNote)
					{
			...
			if(strumline.isPlayer && !strumline.autoplay)
			{
	...
	function noteCheck(keyP:Bool, strumline:Strumline, note:Note):Void // sorry lol
	{
		if (loadRep)
		{
			if (keyP)
				goodNoteHit(strumline, note);
			else if (!newInput) 
				badNoteCheck();
	...
	function goodNoteHit(strumline:Strumline, note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			//trace('note type was ${note.noteType}');
	...
	function killNote(strumline:Strumline, note:Note)
	{
		note.kill();
	...
	override function beatHit()
	{
		...
		{
			for(strumline in strumLines)
				strumline.allNotes.sort(FlxSort.byY, FlxSort.DESCENDING);
		}
